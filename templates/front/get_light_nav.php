...
<nav class="navbar navbar-default full no-radius primary-color-background ">
    ...
    ?></i>
			...
			<li>
				<a class="btn" href="mailto:support@pubforyou.com?subject=pubforyou.com | Erreur dans l'article '<?php the_title_attribute();
    ?>'&body=Bonjour :) %0AVoici l'erreur que j'ai trouvé...%0A%0A%0A%0A%0A-------%0ALien de l'article : <?php the_permalink() ?>" title="Signaler une erreur dans l'article '<?php the_title_attribute();
    ?>'">
