...
class dhfSortable
{
    public $location = ''; // path to css/js
    public $posttype = '';
    public $title = '';
    public $ppp = ''; // postsperpage
    ...
    public function __construct($posttype, $title, $ppp = -1)
    {
        ...
        add_filter('posts_orderby', [$this, 'dhf_order_posts']);
        add_action('admin_menu', [$this, 'dhf_enable_sort']);
        add_action('admin_print_scripts', [$this, 'dhf_sort_scripts']);
        add_action('wp_ajax_dhf_sort', [$this, 'dhf_save_sort_order']);
    }

    /**
     * Alter the query on front and backend to order posts as desired.
     */
    public function dhf_order_posts($orderby)
    {
        global $wpdb;
     ...
     * Add Sort menu.
     */
    public function dhf_enable_sort()
    {
        add_theme_page('edit.php?post_type='.$this->posttype, 'Sort Posts', 'Sort Order', 'edit_posts', 'sort_'.$this->posttype, [$this, 'dhf_sort']);
    }
     ...
     */
    public function dhf_sort_scripts()
    {
        global $pagenow;
     ...
     */
    public function dhf_save_sort_order()
    {
        global $wpdb;
        ...
        foreach ($order as $post_id) {
            $wpdb->update($wpdb->posts, ['menu_order' => $counter], ['ID' => $post_id]);
            $counter++;
        }
