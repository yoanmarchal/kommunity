...
function site_router(){
	...
	if(count($url) == 1 && $url[0] == 'login'){
		...
		die();
	} else	if(count($url) == 1 && $url[0] == 'reset'){
		require 'tpl-reset.php';
		die();
	} else	if(count($url) == 1 && $url[0] == 'edit-avatar'){
...
function add_my_post_types_to_query( $query ) {
	if ( is_home() && $query->is_main_query() )
		$query->set( 'post_type', array( 'bonplan', 'video', 'photo' ) );
	return $query;
}
...
function custom_post_author_archive($query) {
    if ($query->is_author)
        $query->set( 'post_type', array('bonsplanssorties', 'video') );
...
function date_piker() {
	...
	function register_my_script() {
		wp_register_script('my-script', plugins_url('date.js', __FILE__), array('jquery'), '1.0', true);
	}
