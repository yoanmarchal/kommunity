...
<?php if (is_user_logged_in()) {
    ...
    print_r($VideoObject);
    echo '</pre>';
    echo $VideoObject[0]['url'];
    echo $VideoObject[0]['thumbnail'];
	...
	<div id="primary-form-video" class="add-top">
		<form action="<?php echo add_query_arg('do', 'add_video');
    ?>" id="primaryPostForm" method="POST" role="form">
    ...
    ?>" class="form-control required" placeholder="Titre"/>
				...
				<fieldset class="form-group">
					...
					<input type="url" name="videoUrl" id="videoUrl" class="form-control required" placeholder="http://youtu.be/i3Jv9fNPjgk" value="<?php if (isset($_POST['videoUrl'])) {
    echo $_POST['videoUrl'];
}
    ?>" />
