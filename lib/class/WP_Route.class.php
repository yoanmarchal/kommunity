...
class WP_Route extends WP_Router_Utility
{
    ...
    public function __construct($id, array $properties)
    {
        ...
        foreach (['path', 'page_callback'] as $property) {
    ...
    {
        if (in_array($property, ['id', 'path', 'page_callback']) && !$value) {
            throw new Exception(sprintf(__('Invalid value for %s. Value may not be empty.', 'wp-router'), $property));
        }
        if (in_array($property, ['query_vars', 'title_arguments', 'page_arguments', 'access_arguments']) && !is_array($value)) {
    ...
    protected function error_403()
    {
        ...
        $args = apply_filters('wp_router_access_denied_args', ['response' => 403]);
