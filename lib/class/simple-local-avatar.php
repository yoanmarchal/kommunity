...
class Simple_Local_Avatars
{
    ...
    public function __construct()
    {
        ...
        add_action('admin_action_remove-simple-local-avatar', [$this, 'action_remove_simple_local_avatar']);
        add_action('wp_ajax_assign_simple_local_avatar_media', [$this, 'ajax_assign_simple_local_avatar_media']);
        add_action('wp_ajax_remove_simple_local_avatar', [$this, 'action_remove_simple_local_avatar']);
    ...
    public function admin_init()
    {
        ...
        if ($old_ops = get_option('simple_local_avatars_caps')) {
            if (!empty($old_ops['simple_local_avatars_caps'])) {
                update_option('simple_local_avatars', ['caps' => 1]);
            }

            delete_option('simple_local_avatar_caps');
        }

        register_setting('discussion', 'simple_local_avatars', [$this, 'sanitize_options']);
        add_settings_field('simple-local-avatars-only', __('Local Avatars Only', 'simple-local-avatars'), [$this, 'avatar_settings_field'], 'discussion', 'avatars', ['key' => 'only', 'desc' => 'Only allow local avatars (still uses Gravatar for default avatars)']);
        add_settings_field('simple-local-avatars-caps', __('Local Upload Permissions', 'simple-local-avatars'), [$this, 'avatar_settings_field'], 'discussion', 'avatars', ['key' => 'caps', 'desc' => 'Only allow users with file upload capabilities to upload local avatars (Authors and above)']);
    }
    ...
    public function admin_enqueue_scripts($hook_suffix)
    {
        ...
        wp_enqueue_script('simple-local-avatars', plugins_url('', __FILE__).'/simple-local-avatars'.$script_name_append.'.js', ['jquery'], false, true);
        wp_localize_script('simple-local-avatars', 'i10n_SimpleLocalAvatars', [
            'user_id'           => $user_id,
    ...
    {
        ?>
    <h3><?php _e('Avatar', 'simple-local-avatars');
        ?></h3>
    
    <table class="form-table">
        <tr>
            <th scope="row"><label for="simple-local-avatar"><?php _e('Upload Avatar', 'simple-local-avatars');
        ?></label></th>
        ...
        if ($upload_rights) {
            ...
            ?>
                    <p style="display: inline-block; width: 26em;">
                        <span class="description"><?php _e('Choose an image from your computer:');
            ?></span><br />
                    ...
                    <p>
                        <?php if (current_user_can('upload_files') && did_action('wp_enqueue_media')) : ?><a href="#" class="button hide-if-no-js" id="simple-local-avatar-media"><?php _e('Choose from Media Library', 'simple-local-avatars');
            ?></a> &nbsp;<?php endif;
            ?>
                        <a href="<?php echo $remove_url;
            ?>" class="button item-delete submitdelete deletion" id="simple-local-avatar-remove"<?php if (empty($profileuser->simple_local_avatar)) {
    echo ' style="display:none;"';
}
            ?>><?php _e('Delete local avatar', 'simple-local-avatars');
            ?></a>
                    </p>
            <?php
        ...
        endforeach;
        ?>
                    <p class="description"><?php _e('If the local avatar is inappropriate for this site, Gravatar will be attempted.');
        ?></p>
                </fieldset></td>
        </tr>
    </table>
    ...
    public function edit_user_profile_update($user_id)
    {
        ...
        if (!empty($_FILES['simple-local-avatar']['name'])) :
            ...
            add_filter('upload_size_limit', [$this, 'upload_size_limit']);

        $this->user_id_being_edited = $user_id; // make user_id known to unique_filename_callback function
            $avatar = wp_handle_upload($_FILES['simple-local-avatar'], [
                'mimes'                     => [
        ...
        remove_filter('upload_size_limit', [$this, 'upload_size_limit']);
...
function get_simple_local_avatar($id_or_email, $size = 96, $default = '', $alt = '')
{
    ...
    if (empty($avatar)) {
        remove_action('get_avatar', [$simple_local_avatars, 'get_avatar']);
        $avatar = get_avatar($id_or_email, $size, $default, $alt);
        add_action('get_avatar', [$simple_local_avatars, 'get_avatar']);
    }
